...
- **主身份是"一个实体"的计量单位。** 因为它对应一个现实实体且每个实体仅一个，它是防女巫、信用与"一人一票"的基础。
- 同一用户的两个身份，对 dApp 和其他用户而言，**与两个不同的人无异**。

### 主身份的实体：个人或组织

主身份代表一个**实体（实体）**，分两类——**每个实体有且仅有一个主身份**：

| 实体 | 原文 | 含义 |
|---|---|---|
| **个人** | **个人** | 自然人——一个人类 |
| **组织** | **组织** | 法人实体——**公司**或**政府部门** |

**组织主身份正是荣誉的*发行方*：** 它们持有基金会的**根证书**并发行荣誉（[荣誉与声誉 §4](HONOR.md)）——根证书类型（**政府 / 基金会 / 其他机构**）*就是*组织的种类。**个人（自然人）则是*接收方*。**
...
> **每一次交互都以身份为单位；账号绝不暴露。** 与 dApp（及外部服务）的一切交互都**以身份进行**——参与其中的是身份。**账号被严格保密**，因为它**绑定了你的主身份**（你的真实 / 物理身份）：泄露账号就等于泄露真实的人。因此**账号隐私就是个人隐私**，且这是硬性保证——dApp 永远不会收到、也无法推导出某个身份背后的账号。
...
> **initData 按身份区分。** 身份模型上线后，dApp 收到的已签名 `initData`（[登录与注册](AUTH.md)）标识的是**当前活动的身份**。切换身份会得到不同的 `initData`。请围绕身份而非"人"来设计你的账户模型。

---

## 3. 荣誉汇总到主身份

荣誉与凭证（[荣誉与声誉](HONOR.md)）**绑定到获得它的身份**，但**一个人的所有荣誉都会向上汇总到其主身份**——即人这一层级的履历。由于主身份每人仅一个，这让人层面的事实**无法规避**：

- 在任何人格上获得的**不良记录**，仍会在主身份处显现——用户无法通过切换身份来逃避。
- **一人一票**在主身份处计票——多余的身份不会带来额外票数（见 [社区治理](GOVERNANCE.md)）。
- **链上信用**在人这一层级评估。

该汇总是**向内的**——是用户自己的合并视图与人层面的强制执行。它**绝不会**作为人格之间的公开关联暴露给其他用户。

> **可在任何身份下领取——发行方对你的主身份无感。** 你以**当前所处的任意身份**领取荣誉；发行方向**该人格**铸造，且**看不到你的主身份**。它仍会汇总到你的主身份（在人这一层级查询信用/声誉时能查到），因此你**同时**获得两点：**防攻击**（无法通过切换身份来逃避或复制记录）**与隐私**（发行方永远不知道你真实是谁）。
...
如此**隐私与信任兼得**：dApp 只得知*"这个匿名用户达标"*，绝不知道*他是谁*。**担保（[担保支付](ESCROW.md)）是旗舰场景**——担保交易 dApp 可要求 ZK 证明的信用 / 声誉门槛，同时让交易者保持匿名。
...
| 属性 | 机制 |
|---|---|
| **账号隐私（严格）** | 账号**绝不**暴露给 dApp 或交易对手——它绑定了你的**主身份**（真实身份），泄露它就等于泄露真实的人。dApp 只看到身份的 `uid`/昵称；登录标识（邮箱/手机号）从不暴露。 |
...
- [ ] 荣誉向主身份汇总（人层面 rollup）——见 [荣誉与声誉](HONOR.md)
