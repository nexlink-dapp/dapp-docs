...
>
> 在外部浏览器（Chrome、Safari 等）中，`window.NexlinkApp` 为 `undefined`——参见[第 3 节](#3-login-flow-general-browser-qr-code-scan)了解二维码回退方案。
...
| 步骤 | 文件 |
|---|---|
| init_data API 调用 | `nexlink/lib/pages/dapp/dapp_apis.dart` → `DappApis.initData()` |
| init_data 缓存 | `nexlink/lib/pages/dapp/dapp_browser/dapp_init_data_cache.dart` |
...
| 方面 | 详情 |
|---|---|
| 存储 | Redis `SET NX`，TTL 匹配 auth_date 最大有效期 |
...
- [x] Danbao OAuth2 服务器（password、refresh_token、authorization_code 授权）
- [x] Danbao 用户/管理员登录接口
...
- [ ] 为 NULL password_hash 加固 `Authenticate`——在密码登录时拒绝无密码用户（第 19 节）
...
- [Nexlink Mini App 规范](../docs/NEXLINK_MINIAPP_SPEC.md)——完整的 JS SDK 和 API 规范
- [Nexlink 架构](../docs/architecture-README.md)——应用版本管理和升级系统
